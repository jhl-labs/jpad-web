  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold">워크스페이스 관리</h1>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: "var(--sidebar-bg)", borderBottom: "1px solid var(--border)" }}>
              <th className="text-left px-4 py-3 font-medium" style={{ color: "var(--muted)" }}>워크스페이스</th>
              <th className="text-left px-4 py-3 font-medium hidden md:table-cell" style={{ color: "var(--muted)" }}>공개여부</th>
              <th className="text-left px-4 py-3 font-medium hidden md:table-cell" style={{ color: "var(--muted)" }}>멤버</th>
              <th className="text-left px-4 py-3 font-medium hidden md:table-cell" style={{ color: "var(--muted)" }}>페이지</th>
              <th className="text-left px-4 py-3 font-medium hidden sm:table-cell" style={{ color: "var(--muted)" }}>생성일</th>
                ...
                >
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span
                      ...
                      style={{
                        background: ws.visibility === "public" ? "#22c55e20" : "var(--sidebar-bg)",
                        color: ws.visibility === "public" ? "#22c55e" : "var(--muted)",
                      }}
                    >
                      {ws.visibility === "public" ? "공개" : "비공개"}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 hidden sm:table-cell" style={{ color: "var(--muted)" }}>
                    {new Date(ws.createdAt).toLocaleDateString("ko")}
                  </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-4">
          <span className="text-sm" style={{ color: "var(--muted)" }}>
            {offset + 1}-{Math.min(offset + PAGE_SIZE, total)} / {total}개
          </span>
        ...
        >
          ...
          >
            ...
            <p className="text-sm mb-1">
              <strong>{confirmDelete.name}</strong> ({confirmDelete.slug})
            </p>
            <p className="text-sm mb-4" style={{ color: "#ef4444" }}>
              이 워크스페이스의 모든 데이터(페이지, 멤버십, 첨부파일 등)가 영구적으로 삭제됩니다.
            </p>
