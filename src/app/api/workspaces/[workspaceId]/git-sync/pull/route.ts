...
import { pullFromRemote } from "@/lib/git/remote";
...
export async function POST(
  ...
  try {
    ...
    const allowed = await rateLimitRedis(`git-sync:pull:${user.id}`, 10, 60_000);
    if (!allowed) {
    ...
    if (syncMode === "push_only") {
      return NextResponse.json(
        { error: "Sync mode is set to push_only" },
        { status: 400 }
      );
    }
    ...
    try {
      const result = await pullFromRemote(workspaceId, {
      ...
      await recordAuditLog({
        action: "workspace.git_sync.pull",
      ...
      return NextResponse.json(
        { error: `Pull failed: ${errorMessage}`, logId: logEntry.id },
        { status: 500 }
      );
    }
  } catch (error) {
    ...
    logError("git_sync.pull.unhandled_error", error, {}, req);
